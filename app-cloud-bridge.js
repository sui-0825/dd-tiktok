...
const diag=[];
const ENTRY_CURSOR_KEY='dd_entry_records_cursor_v1';
...
async function pushEntryById(entryId){
 ...
 try{
  ...
  state.entryCursor=String(saved?.updated_at||now);
...
async function runReceiveCycle(reason='timer'){
 // Ver25.57: app_entry_records is independent from app_snapshots revision.
 // Always receive entry-level changes first, then inspect the legacy snapshot revision.
 const entryResult=await pullEntryRecords(reason);
 const snapshotResult=await checkForRemoteUpdates(reason);
 return {ok:Boolean(entryResult?.ok||snapshotResult?.ok),entries:Number(entryResult?.count||0),snapshot:snapshotResult};
}
function startAutomaticReceive(){
 clearInterval(state.receiveTimer);
 if(state.accessStatus!=='approved')return;
 runReceiveCycle('startup').catch(console.warn);
 state.receiveTimer=setInterval(()=>runReceiveCycle('interval').catch(console.warn),3000);
}
...
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&state.accessStatus==='approved'){startAutomaticReceive();heartbeat().catch(console.warn);runReceiveCycle('visible')}else{stopAutomaticReceive()}});
window.addEventListener('focus',()=>{if(state.accessStatus==='approved'){startAutomaticReceive();heartbeat().catch(console.warn);runReceiveCycle('focus')}});
window.addEventListener('pageshow',()=>{if(state.accessStatus==='approved'){startAutomaticReceive();runReceiveCycle('pageshow')}});
