/* D&D❀TikTok Ver26.00 historical-gap repair + multi-device safe durable delta sync */
(()=>{
'use strict';
...
const diag=[];
const ENTRY_CURSOR_KEY='dd_entry_records_cursor_v2', META_CURSOR_KEY='dd_meta_cursor_v1', ENTRY_HISTORY_REPAIR_KEY='dd_entry_history_repair_v2600';
...
function setStatus(status,mode=state.mode,error='',stage=state.stage){state.status=status;state.mode=mode;if(error)state.lastError=String(error);state.stage=stage||'';document.documentElement.dataset.storageMode=mode;document.documentElement.dataset.cloudStage=state.stage;emit()}
...
async function hydrateDurableCursors(){
 ...
 state.historyRepairComplete=Boolean(durable.historyRepairV2600);
 if(state.historyRepairComplete){try{localStorage.setItem(ENTRY_HISTORY_REPAIR_KEY,'complete')}catch(_){}}
...
function recordClock(x){return Math.max(Number(x?.editedAt||0),Number(x?.updatedAt||0),Number(x?.cancelledAt||0),Number(x?.canceledAt||0),Number(x?.deletedAt||0),Number(x?.timestamp||0),Number(x?.createdAt||0))}
...
function financeClock(db,kind,key=''){
 const clocks=db?._syncMeta?.finance||{};
 if(kind==='monthly')return Number(clocks.monthly?.[key]||0);
 if(kind==='targets')return Number(clocks.targets?.[key]||0);
 if(kind==='initialInvestment')return Number(clocks.initialInvestment||0);
 return 0;
}
function mergeFinance(remote={},local={},remoteRoot={},localRoot={}){
 const out={...(remote||{}),...(local||{})};
 out.monthly={};out.targets={};
 for(const key of new Set([...Object.keys(remote?.monthly||{}),...Object.keys(local?.monthly||{})])){
  const rc=financeClock(remoteRoot,'monthly',key),lc=financeClock(localRoot,'monthly',key);
  out.monthly[key]=lc>rc?local?.monthly?.[key]:remote?.monthly?.[key];
 }
 for(const key of new Set([...Object.keys(remote?.targets||{}),...Object.keys(local?.targets||{})])){
  const rc=financeClock(remoteRoot,'targets',key),lc=financeClock(localRoot,'targets',key);
  out.targets[key]=lc>rc?local?.targets?.[key]:remote?.targets?.[key];
 }
 const ric=financeClock(remoteRoot,'initialInvestment'),lic=financeClock(localRoot,'initialInvestment');
 out.initialInvestment=lic>ric?Number(local?.initialInvestment||0):Number(remote?.initialInvestment||0);
 return out;
}
function mergeSnapshots(remote={},local={}){
 ...
 out._syncMeta={...(remote._syncMeta||{}),...(local._syncMeta||{}),finance:{...(remote._syncMeta?.finance||{}),...(local._syncMeta?.finance||{}),monthly:{...(remote._syncMeta?.finance?.monthly||{}),...(local._syncMeta?.finance?.monthly||{})},targets:{...(remote._syncMeta?.finance?.targets||{}),...(local._syncMeta?.finance?.targets||{})}}};
 out.finance=mergeFinance(remote.finance||{},local.finance||{},remote,local);
...
async function ownerEmergencyUnlock(pin){
  ...
  state.dirty=false;state.dirtySeq=0;
  try{localStorage.removeItem(DIRTY_KEY);localStorage.removeItem(DIRTY_SEQ_KEY)}catch(_){}
  setStatus('差分共有へ接続済み','cloud','','delta-ready');
  await bootstrapParentOnceIfNeeded();
  startPresence();
  refreshUI();
  return true;
}
 ...
 state.accessTimer=setInterval(async()=>{
  try{
   ...
   if(state.accessStatus==='approved'){
    ...
    state.dirty=false;state.dirtySeq=0;
    try{localStorage.removeItem(DIRTY_KEY);localStorage.removeItem(DIRTY_SEQ_KEY)}catch(_){}
    setStatus('チェックイン承認済み・差分共有接続','cloud','','delta-ready');
    await bootstrapParentOnceIfNeeded();
    await repairHistoricalEntryGapOnce();startPresence();startAutomaticReceive();refreshUI();
...
async function requestAccess(displayName){
 ...
 if(state.accessStatus==='approved'){
  ...
  state.dirty=false;state.dirtySeq=0;
  try{localStorage.removeItem(DIRTY_KEY);localStorage.removeItem(DIRTY_SEQ_KEY)}catch(_){}
  setStatus(state.role==='owner'?'管理者として差分共有へ接続':'承認済み・差分共有へ接続','cloud','','delta-ready');
  await bootstrapParentOnceIfNeeded();
  await repairHistoricalEntryGapOnce();
...
async function bootstrapParentOnceIfNeeded(){
 ...
 const localDevices=Array.isArray(window.db.devices)?window.db.devices.length:0;
 const key=`${PARENT_BOOTSTRAP_KEY}:${state.workspaceId}`;
 // Androidなど、すでに本体データを十分持つ端末では親データを読まない。
 if(localEntries>=1000&&localDevices>=100){try{localStorage.setItem(key,'complete')}catch(_){};return {ok:true,skipped:true,reason:'local_complete'}}
 try{
  setStatus('初回データをクラウドから受信中…','cloud','','bootstrap-parent');
  const rows=await rest(`app_snapshots?workspace_id=eq.${encodeURIComponent(state.workspaceId)}&select=revision,payload,updated_at&limit=1`,{timeoutMs:90000});
  const row=Array.isArray(rows)?rows[0]:null;
  const payload=row?.payload;
  const remoteEntries=Array.isArray(payload?.entries)?payload.entries.length:0;
  const remoteDevices=Array.isArray(payload?.devices)?payload.devices.length:0;
  if(!payload||typeof payload!=='object'||remoteEntries<1||remoteDevices<1)throw new Error('クラウド親データが空、または形式が不正です');
  // 空に近い端末だけを復元対象にする。既存データを小さい親データで上書きしない。
  if(localEntries>0&&localEntries>=remoteEntries){try{localStorage.setItem(key,'complete')}catch(_){};return {ok:true,skipped:true,reason:'local_newer'}}
  const localUser=getLocalUserName()||String(window.db.currentUser||'');
  const localMembers=Array.isArray(window.db.members)?window.db.members.slice():[];
  state.applying=true;
  window.db=typeof structuredClone==='function'?structuredClone(payload):JSON.parse(JSON.stringify(payload));
  if(typeof window.recalculateStoredEntries==='function')window.recalculateStoredEntries(window.db);
  mergeSharedMembers(localMembers);
  if(localUser)window.db.currentUser=localUser;
  state.revision=Number(row.revision||state.revision||0);
  const baseCursor=String(row.updated_at||'1970-01-01T00:00:00.000Z');
  state.entryCursor=baseCursor;
  try{
   ...
   localStorage.setItem(ENTRY_CURSOR_KEY,baseCursor);
   localStorage.setItem(key,'complete');
  }catch(_){}
  state.applying=false;
  refreshUI();
  setStatus(`初回データ受信完了（${remoteDevices}台・${remoteEntries}件）`,'cloud','','bootstrap-complete');
  addDiag('BOOTSTRAP_COMPLETE',`devices=${remoteDevices} entries=${remoteEntries} revision=${state.revision}`);
  return {ok:true,updated:true,devices:remoteDevices,entries:remoteEntries,revision:state.revision};
 }catch(e){
  state.applying=false;
  addDiag('BOOTSTRAP_ERROR',e?.message||String(e));
  setStatus('初回データ受信に失敗しました','error',e?.message||String(e),'bootstrap-error');
...
async function deleteEntryById(entryId,deviceId=''){
 if(state.accessStatus!=='approved'||!state.workspaceId||!state.user)return {ok:false,reason:'not_ready'};
 const id=String(entryId||'');if(!id)return {ok:false,reason:'entry_id_required'};
 const now=new Date().toISOString();
 try{
...
function buildMetaPayload(){if(!window.db||typeof window.db!=='object')return {};const x=typeof structuredClone==='function'?structuredClone(window.db):JSON.parse(JSON.stringify(window.db));delete x.entries;delete x.currentUser;return x}
async function pushMeta(){
 if(state.accessStatus!=='approved'||!state.workspaceId||!state.user||state.applying)return {ok:false,reason:'not_ready'};
 try{
  const now=new Date().toISOString();
  const localPayload=buildMetaPayload();
  const currentRows=await rest(`app_meta_state?workspace_id=eq.${encodeURIComponent(state.workspaceId)}&select=payload,updated_at&limit=1`,{timeoutMs:20000});
  const current=Array.isArray(currentRows)?currentRows[0]:null;
  const payload=mergeSnapshots(current?.payload||{},localPayload);delete payload.entries;delete payload.currentUser;
  const rows=await rest('app_meta_state?on_conflict=workspace_id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({workspace_id:state.workspaceId,payload,updated_by:state.user.id,updated_at:now}),timeoutMs:20000});
...
async function pullMeta(){
 ...
 const cursor=state.metaCursor||(()=>{try{return localStorage.getItem(META_CURSOR_KEY)||'1970-01-01T00:00:00.000Z'}catch(_){return '1970-01-01T00:00:00.000Z'}})();
 try{const rows=await rest(`app_meta_state?workspace_id=eq.${encodeURIComponent(state.workspaceId)}&updated_at=gt.${encodeURIComponent(cursor)}&select=payload,updated_at&limit=1`,{timeoutMs:20000});const row=Array.isArray(rows)?rows[0]:null;if(!row?.payload)return {ok:true,updated:false};const localUser=window.db?.currentUser||'';state.applying=true;const merged=mergeSnapshots(row.payload,window.db||{});window.db={...(window.db||{}),...merged,entries:Array.isArray(window.db?.entries)?window.db.entries:[],currentUser:localUser};state.metaCursor=String(row.updated_at||cursor);try{const saved=typeof window.compactDBForStorage==='function'?window.compactDBForStorage(window.db):window.db;localStorage.setItem(window.KEY||'dd_tiktok_app_v14_production',JSON.stringify(saved));localStorage.setItem(META_CURSOR_KEY,state.metaCursor)}catch(_){}state.applying=false;mirrorLongTermStorage();refreshUI();return {ok:true,updated:true}}catch(e){state.applying=false;return {ok:false,reason:e?.message||String(e)}}
}

function entryPullCursor(cursor,overlapMs=30000){
 const t=Date.parse(String(cursor||''));
 if(!Number.isFinite(t))return '1970-01-01T00:00:00.000Z';
 return new Date(Math.max(0,t-overlapMs)).toISOString();
}
function applyEntryRecordRow(row,{preferNewerLocal=false}={}){
 const id=String(row?.entry_id||'');
 if(!id)return {changed:false,entryChanged:false,deviceChanged:false};
 let entryChanged=false,deviceChanged=false;
 const entries=Array.isArray(window.db?.entries)?window.db.entries:(window.db.entries=[]);
 const pos=entries.findIndex(e=>String(e?.id||'')===id);
 if(row.deleted){
  if(pos>=0){entries.splice(pos,1);entryChanged=true}
 }else if(row.entry_data&&typeof row.entry_data==='object'){
  if(pos>=0){
   const local=entries[pos],remote=row.entry_data;
   const keepLocal=preferNewerLocal&&recordClock(local)>recordClock(remote);
   if(!keepLocal&&recordSignature(local)!==recordSignature(remote)){entries[pos]=remote;entryChanged=true}
  }else{entries.push(row.entry_data);entryChanged=true}
 }
 const dev=row.device_data;
 if(dev&&typeof dev==='object'){
  const devices=Array.isArray(window.db?.devices)?window.db.devices:(window.db.devices=[]);
  const did=String(dev.id||'');
  const dp=devices.findIndex(d=>String(d?.id||'')===did);
  if(dp>=0){
   const localDev=devices[dp];
   const keepLocalDev=preferNewerLocal&&recordClock(localDev)>recordClock(dev);
   if(!keepLocalDev&&recordSignature(localDev)!==recordSignature(dev)){devices[dp]=dev;deviceChanged=true}
  }else if(did){devices.push(dev);deviceChanged=true}
 }
 return {changed:entryChanged||deviceChanged,entryChanged,deviceChanged};
}

// Ver26.00: Ver25.98以前の受信カーソル飛びで既にできた「過去の穴」を1回だけ修復する。
// app_entry_records は entry_id ごとに最新状態を1行保持するため、全件をページング照合しても
// 入力ルールや共有方式は変えず、端末内に無い/古い入力だけを補える。
async function repairHistoricalEntryGapOnce(){
 if(state.accessStatus!=='approved'||!state.workspaceId||document.visibilityState==='hidden')return {ok:false,reason:'not_ready'};
 if(state.historyRepairComplete)return {ok:true,skipped:true};
 try{if(localStorage.getItem(ENTRY_HISTORY_REPAIR_KEY)==='complete'){state.historyRepairComplete=true;return {ok:true,skipped:true}}}catch(_){}
 if(state.receiveBusy||state.applying)return {ok:false,reason:'busy'};
 const cutoff=new Date().toISOString(),pageSize=500,maxPages=50;
 let offset=0,total=0,changed=0,maxUpdated=state.entryCursor||'',completed=false;
 state.receiveBusy=true;
 setStatus('過去の入力差分を1度だけ照合中…','cloud','','history-repair');
 try{
  state.applying=true;
  for(let page=0;page<maxPages;page++){
   const rows=await rest(`app_entry_records?workspace_id=eq.${encodeURIComponent(state.workspaceId)}&updated_at=lte.${encodeURIComponent(cutoff)}&select=entry_id,entry_data,device_data,deleted,updated_at&order=updated_at.asc,entry_id.asc&limit=${pageSize}&offset=${offset}`,{timeoutMs:30000});
   if(!Array.isArray(rows)||!rows.length){completed=true;break;}
   for(const row of rows){
    const applied=applyEntryRecordRow(row,{preferNewerLocal:true});
    if(applied.changed)changed++;
    maxUpdated=newerCursor(maxUpdated,String(row?.updated_at||''));
   }
   total+=rows.length;offset+=rows.length;
   if(rows.length<pageSize){completed=true;break;}
  }
  if(!completed)throw new Error('履歴件数が安全上限を超えました。次回も照合を継続します');
  if(typeof window.recalculateStoredEntries==='function')window.recalculateStoredEntries(window.db);
  state.entryCursor=newerCursor(state.entryCursor,maxUpdated);
  try{const saved=typeof window.compactDBForStorage==='function'?window.compactDBForStorage(window.db):window.db;localStorage.setItem(window.KEY||'dd_tiktok_app_v14_production',JSON.stringify(saved))}catch(_){ }
  try{if(state.entryCursor)localStorage.setItem(ENTRY_CURSOR_KEY,state.entryCursor)}catch(_){ }
  state.historyRepairComplete=true;
  try{localStorage.setItem(ENTRY_HISTORY_REPAIR_KEY,'complete')}catch(_){ }
  state.applying=false;
  await window.DDLongTermStorage?.saveNow?.().catch(()=>{});
  refreshUI();
  addDiag('HISTORY_REPAIR_COMPLETE',`checked=${total} changed=${changed} cursor=${state.entryCursor||'-'}`);
  setStatus(changed?`過去差分を修復しました（${changed}件）`:'過去差分の照合完了','cloud','','history-repair-complete');
  return {ok:true,total,changed};
 }catch(e){
  state.applying=false;
  addDiag('HISTORY_REPAIR_ERROR',e?.message||String(e));
  setStatus('過去差分の照合を次回再試行します','cloud',e?.message||String(e),'history-repair-error');
  return {ok:false,reason:e?.message||String(e)};
...
async function pullEntryRecords(reason='timer'){
 ...
 // Ver26.00: 端末時計の数秒差や同一時刻境界での取りこぼしを避けるため30秒だけ重ねて確認。
 // entry_idで統合するので重複登録にはならない。
 const queryCursor=entryPullCursor(cursor,30000);
 state.receiveBusy=true;
 try{
  const rows=await rest(`app_entry_records?workspace_id=eq.${encodeURIComponent(state.workspaceId)}&updated_at=gt.${encodeURIComponent(queryCursor)}&select=entry_id,entry_data,device_data,deleted,updated_at&order=updated_at.asc&limit=500`,{timeoutMs:20000});
  if(!Array.isArray(rows)||!rows.length)return {ok:true,count:0,changed:0};
  state.applying=true;
  let changed=0,maxUpdated=state.entryCursor;
  for(const row of rows){
   if(applyEntryRecordRow(row).changed)changed++;
   maxUpdated=newerCursor(maxUpdated,String(row?.updated_at||''));
  }
  state.entryCursor=maxUpdated;
  if(changed&&typeof window.recalculateStoredEntries==='function')window.recalculateStoredEntries(window.db);
  if(changed){
   try{const saved=typeof window.compactDBForStorage==='function'?window.compactDBForStorage(window.db):window.db;localStorage.setItem(window.KEY||'dd_tiktok_app_v14_production',JSON.stringify(saved))}catch(_){}
  }
  ...
  state.applying=false;
  mirrorLongTermStorage();
  if(changed){refreshUI();setStatus(`入力${changed}件を受信しました`,'cloud','','entry-record-received')}
...
async function inspectAuthIdentity(){
 if(!state.user)await ensureAuth();
 if(!state.workspaceId)await verifyWorkspace();
 const decodeJwt=(token)=>{try{const part=String(token||'').split('.')[1]||'';const b64=part.replace(/-/g,'+').replace(/_/g,'/');const txt=decodeURIComponent(atob(b64).split('').map(c=>'%' + c.charCodeAt(0).toString(16).padStart(2,'0')).join(''));return JSON.parse(txt)}catch(_){return {}}};
 const claims=decodeJwt(state.accessToken);
 let membershipRows=[],membershipError='',profileRows=[],profileError='';
 try{membershipRows=await rest(`workspace_members?workspace_id=eq.${encodeURIComponent(state.workspaceId)}&user_id=eq.${encodeURIComponent(state.user.id)}&select=*`,{timeoutMs:20000})}catch(e){membershipError=String(e?.message||e)}
 try{profileRows=await rest(`profiles?user_id=eq.${encodeURIComponent(state.user.id)}&select=*`,{timeoutMs:20000})}catch(e){profileError=String(e?.message||e)}
 return {
  appUserId:String(state.user?.id||''),
  jwtSub:String(claims?.sub||''),
  jwtRole:String(claims?.role||''),
  jwtExp:Number(claims?.exp||0),
  workspaceId:String(state.workspaceId||''),
  localRole:String(state.role||''),
  localAccessStatus:String(state.accessStatus||''),
